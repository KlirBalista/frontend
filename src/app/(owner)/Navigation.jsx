...
const Navigation = ({ user }) => {
  ...
  useEffect(() => {
    const fetchFacility = async () => {
      try {
        const response = await axios.get("/api/owner/birthcare");
        setFacility(response.data);
      } catch (error) {
        if (error.response?.status !== 404) {
          console.error("Error fetching facility:", error);
        }
        // 404 means no facility registered yet
        setFacility(null);
      }
    };
  ...
  return (
    <>
          ...
          <div className="relative">
            ...
            >
              <div className="flex-shrink-0">
                <div className="h-10 w-10 rounded-full flex items-center justify-center font-medium bg-[#FDB3C2] text-[#A41F39] shadow-sm">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-3 bottom-3 left-0 z-40 bg-gradient-to-b from-[#BF3853] to-[#A41F39] shadow-xl transition-transform duration-300 ease-in-out ml-3 mr-3 rounded-3xl ${
          isMobile
            ? isOpen
              ? "translate-x-0 w-72"
              ...
              {navItems.map((item, index) => (
                <li key={item.name}>
                  {item.submenu ? (
                    ...
                    >
                      <div className="w-8 h-8 mr-3 rounded-lg bg-white/30 flex items-center justify-center">
                        <svg
                          className="w-4 h-4"
                          fill="none"
                          stroke="currentColor"
